...
public class RepeatWeeklyTransferFunds {
	...
	public RepeatWeeklyTransferFunds RepeatWeeklyTransferFunds() throws Exception {
		iOSLogs.setupTest("Repeat Weekly Transfer Funds Test");
		
		iOSLogs.CapturedLogs(info, "Repeat transfer flow test");
...
//		}
		...
		if (WeeklyRepeatSchedule != null && WeeklyRepeatSchedule.isEnabled()) {
			...
			iOSLogs.CapturedLogs(pass, "Weekly Repeat schedule clicked");
		} else {
			iOSLogs.CapturedLogs(fail, "Weekly Repeat schedule not found");
		}
		...
		if (SaturdayWeekly != null && SaturdayWeekly.isEnabled()) {
			...
			iOSLogs.CapturedLogs(pass, "Saturday clicked");
		} else {
			iOSLogs.CapturedLogs(fail, "Saturday not found");
		}
		
		Thread.sleep(3000);
		WebElement NextButtonDaysScreen = elements.NextTransferFundsButtonTransferTo();
		if (NextButtonDaysScreen != null && NextButtonDaysScreen.isEnabled()) {
			NextButtonDaysScreen.click();
			iOSLogs.CapturedLogs(pass, "Next button days screen clicked");
		} else {
			iOSLogs.CapturedLogs(fail, "Next button days screen not found");
		}
