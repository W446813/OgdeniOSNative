...
public class RepeatDailyTransferFunds {
	...
	public RepeatDailyTransferFunds RepeatDailyTransferFunds() throws Exception {
		iOSLogs.setupTest("Repeat Daily Transfer Funds Test");
		
		iOSLogs.CapturedLogs(info, "Repeat daily transfer flow test");
...
//			iOSLogs.CapturedLogs(pass, "% of balance tab clicked");
//		} else {
//			iOSLogs.CapturedLogs(fail, "% of balance tab not found");
//		}
		
		Thread.sleep(3000);
		WebElement EnterPercentTextBox = elements.EnterPercentTextBox();
		if (EnterPercentTextBox != null && EnterPercentTextBox.isEnabled()) {
			EnterPercentTextBox.sendKeys(iOSText.PERCENT_BALANCE_AMOUNT);
			iOSLogs.CapturedLogs(pass, "Percent entered");
		} else {
			iOSLogs.CapturedLogs(fail, "Percent textbox not found");
		}
		...
		if (DoneDatePickerButton != null && DoneDatePickerButton.isEnabled()) {
			...
			iOSLogs.CapturedLogs(pass, "Set start date Next button clicked");
		} else {
			iOSLogs.CapturedLogs(fail, "Set start date Next button not found");
		}
...
//		}
		...
		if (DailyRepeatSchedule != null && DailyRepeatSchedule.isEnabled()) {
			...
			iOSLogs.CapturedLogs(pass, "Daily Repeat schedule clicked");
		} else {
			iOSLogs.CapturedLogs(fail, "Daily Repeat schedule not found");
		}
